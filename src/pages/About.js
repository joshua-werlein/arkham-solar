    ...
    <main>
      ...
      <section style={{ position: 'relative', height: 360, display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, paddingBottom: 64 }}>
          <div className="section-label" style={{ marginBottom: 8, color: '#D6B64A', fontSize: 15, fontWeight: 700 }}>Our Story</div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(48px, 6vw, 80px)', color: 'var(--hero-heading)', letterSpacing: 3 }}>ABOUT US</h1>
        </div>
      </section>
      ...
      <section style={{ padding: '100px 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container grid-collapse" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center', position: 'relative', zIndex: 1 }}>
          <div>
            <img src={logo} alt="Arkham Enterprises" style={{ width: '80%', filter: 'drop-shadow(0 0 60px var(--red-glow))' }} />
          </div>
          <div>
            <div className="section-label" style={{ marginBottom: 12, color: 'var(--gold)', fontSize: 14, fontWeight: 700 }}>Our Mission</div>
      ...
      <section style={{ background: 'var(--charcoal)', padding: '100px 0' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            <div className="section-label" style={{ marginBottom: 12, color: 'var(--gold)', fontSize: 14, fontWeight: 700 }}>Core Values</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 32 }}>
            {values.map(({ icon: Icon, title, desc }) => (
              <div key={title} style={{ padding: '40px 32px', background: 'var(--surface)', borderLeft: '2px solid var(--gold)' }}>
                <Icon size={28} color="var(--gold)" strokeWidth={1.5} style={{ marginBottom: 16 }} />
      ...
      <section style={{ padding: '80px 0', borderTop: '1px solid var(--border)' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          <div className="section-label" style={{ marginBottom: 12, color: 'var(--gold)', fontSize: 14, fontWeight: 700 }}>A Division of Arkham Enterprises</div>
          ...
          <p style={{ color: 'var(--copy)', maxWidth: 560, margin: '0 auto 40px', lineHeight: 1.8 }}>
          ...
          <Link to="/services" className="btn-outline" style={{ borderWidth: 2, borderColor: 'var(--gold)', color: 'var(--gold)', fontWeight: 600 }}>View Services <ArrowRight size={16} /></Link>
        </div>
      </section>
    </main>
  );
